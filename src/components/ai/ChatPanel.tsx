  ...
  return (
    ...
    >
      ...
      >
        <header className={`px-4 py-4 ${FLOATING_WIDGET_PANEL_HEADER}`}>
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <h2 className="mt-1 font-semibold text-lg text-text-primary">Chat with Zomer</h2>
            </div>
            <button
              type="button"
              aria-label="Minimize assistant"
              onClick={onClose}
              className="rounded-full border border-border px-3 py-1 font-mono text-text-muted text-xs uppercase tracking-[0.18em] transition hover:border-primary/40 hover:text-primary"
            >
              Hide
            </button>
          </div>
        ...
        {error && (
          <div className="border-border border-b bg-terminal-red/10 px-4 py-3 text-red-200 text-sm">
            {error}
          </div>
        )}
        <ChatMessageList
