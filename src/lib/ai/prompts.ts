...
export function buildGroundedAnswerPrompt(input: {
  ...
  return [
    ...
    "EXCEPTION: Questions about THIS chat session (e.g. 'what did I ask', 'summarize our conversation', 'what were my messages', 'repeat your last answer') — answer using the 'Prior conversation' block below. That block IS authoritative for chat-meta questions.",
    "When the user asks for the latest / most recent / current / newest item (e.g. latest blog, most recent project), read the 'Published' date on each source and pick the one with the most recent date.",
    "For experience entries (no Published date), the retrieved sources are already ordered newest-first — the FIRST experience source IS the current/most recent role.",
    "When listing multiple items chronologically, order them by 'Published' date descending (newest first) unless the user asks otherwise.",
